...
import { inboxmessagelistAction } from '../../../util/action.js';
...
export default class inbox extends Component {
    ...
    async inboxmessagelist() {
        ...
        inboxmessagelistAction(loginData).then(responseJson => {
                    ...
                    <TouchableOpacity onPress={() => this.props.navigation.navigate('InboxView', { messagekey: item.message_id, viewMessageID: item.view_message_id, messageimage: item.gmgt_user_avatar_image, messagetext: item.description, messagetitle: item.subject, replyid: item.replay_id, receiverid: item.receiver_id, userImage: item.current_user_image })}>
                        <Row style={{ borderBottomWidth: 1, borderBottomColor: '#E0DFDF', marginRight: '5%', marginLeft: '5%', height: 95 }}>
                            <Col style={{ width: '21%', justifyContent: 'center', alignItems: 'flex-start' }}>
                                <Col style={{ borderWidth: 1, height: 60, width: 60, borderRadius: 50, alignItems: 'center', justifyContent: 'center' }}>
